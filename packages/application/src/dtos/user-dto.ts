...
export const setAssistantProfileSchema = z.object({
  ...
  workHoursStart: z.number().int().min(0).max(23).nullish(),
  workHoursEnd: z.number().int().min(0).max(23).nullish(),
...
const assistantProfileViewSchema = z.object({
  ...
  workHoursStart: z.number().nullable(),
  workHoursEnd: z.number().nullable(),
